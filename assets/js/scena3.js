...
const createScene = async function () {
  ...
  wallEndRightExtra_bis.position = new BABYLON.Vector3(76, extraY - 1, 55.2);
  ...
  newPerpWallLeft.position = new BABYLON.Vector3(112, extraY, 82);
  ...
  wallEndLeftExtra2.position = new BABYLON.Vector3(148, extraY, 55.2);
  ...
  for (let s = 0; s < totalSteps; s++) {
    ...
    const box = BABYLON.MeshBuilder.CreateBox("stepNeg_" + s, { width: 1, height: h, depth: 15 }, scene);
  ...
  for (let s = 0; s < totalSteps; s++) {
    const h = (s + 1) * (totalHeight / totalSteps);
    const box = BABYLON.MeshBuilder.CreateBox("stepPos_" + s, { width: 1, height: h, depth: 15 }, scene);
  ...
  for (let i = 0; i < 12; i++) {
    ...
    ctxGray.bezierCurveTo(Math.random() * 512, Math.random() * 512, Math.random() * 512, Math.random() * 512, Math.random() * 512, 512);
    ctxGray.stroke();
  }
  ...
  const hexagonMirror = BABYLON.MeshBuilder.CreateCylinder("hexagonMirror", { height: 7, diameter: 8, tessellation: 6 }, scene);
  hexagonMirror.position = new BABYLON.Vector3(147, 2.5, -9);
  ...
  const raisedFloorCollider = BABYLON.MeshBuilder.CreateBox("raisedFloorCollider", { width: 112, height: 0.5, depth: 59 }, scene);
  ...
  for (let s = 0; s < totalStepsSide; s++) {
    ...
    const box = BABYLON.MeshBuilder.CreateBox("stepSide_" + s, { width: spanX, height: h, depth: 1 }, scene);
  ...
  const sideFloorCollider = BABYLON.MeshBuilder.CreateBox("sideFloorCollider", { width: 90, height: 0.5, depth: 40 }, scene);
  ...
  const perpWallTop1 = BABYLON.MeshBuilder.CreateBox("perpWallTop1", { width: 104, height: wallHeightBox + 1, depth: wallThickness + 0.2 }, scene);
  ...
  const connectorWall = BABYLON.MeshBuilder.CreateBox("connectorWall", { width: 0.8, height: wallHeightBox + 100, depth: 53.5 + 30 }, scene);
  ...
  for (let i = 0; i < 7; i++) {
    newColumnPositions.push(172 + i * 18);
  }
  ...
  for (let i = 0; i < newColumnPositions.length - 1; i++) {
    ...
    [15, -16].forEach((zPos) => {
      ...
      const wallBox2 = BABYLON.MeshBuilder.CreateBox("wall2_" + i + zPos, { width: currentWallWidth, height: wallHeight, depth: currentDepth }, scene);
      wallBox2.position = new BABYLON.Vector3(finalMidX, archBaseY2 + wallHeight / 2, finalZ);

      const currentArchDiameter = isLeft ? 9 : 8;
      const archHole2 = BABYLON.MeshBuilder.CreateCylinder("archHole2_" + i + zPos, { height: currentDepth + 0.5, diameter: currentArchDiameter }, scene);
  ...
  const gapFill2 = BABYLON.MeshBuilder.CreateBox("gapFill2", { width: 19, height: wallHeightBox, depth: wallThickness + 0.2 }, scene);
  gapFill2.position = new BABYLON.Vector3(148 + 19 / 2, wallHeightBox / 2, -25.4);
  gapFill2.material = wallMaterial;
  gapFill2.checkCollisions = true;

  const boxFill = BABYLON.MeshBuilder.CreateBox("boxFill", { width: 22, height: 10, depth: 20 }, scene);
  boxFill.position = new BABYLON.Vector3(157, 27, 25.41);
  boxFill.material = wallMaterial;
  boxFill.checkCollisions = true;

  const boxFill2 = BABYLON.MeshBuilder.CreateBox("boxFill2", { width: 22, height: 10, depth: 20 }, scene);
  boxFill2.position = new BABYLON.Vector3(157, 27, -20.39);
