...
//import { tagliaVolteDentroCupola, tagliaCupolaConVaults, createSafeVault, tagliaCupolaConBox } from "./domeUtils.js";
    ...
    BABYLON.SceneLoader.ImportMeshAsync("", "assets/models/", "Columns.glb", scene).then((result) => {
        ...
        for (let i = 0; i < columnPositions.length - 1; i++) {
            ...
            [15, -16].forEach(zPos => {
                ...
                const wallBox = BABYLON.MeshBuilder.CreateBox("wall", { width: currentWallWidth, height: wallHeight, depth: currentDepth }, scene);
                ...
                const archHole = BABYLON.MeshBuilder.CreateCylinder("archHole", { height: currentDepth + 0.5, diameter: currentArchDiameter }, scene);
        ...
        const wallEndRightExtra = BABYLON.MeshBuilder.CreateBox("wallEndrRightExtra", { width: 3.5, height: wallHeightBox + 1, depth: 60 }, scene);
        ...
        const newPerpWallRight = BABYLON.MeshBuilder.CreateBox("newPerpWallRight", { width: 2.1, height: wallHeightBox + 100, depth: 70 }, scene);
        ...
        const rectWallRight = BABYLON.MeshBuilder.CreateBox("rectWallRight", { width: 1.6, height: 10, depth: 14 }, scene);
        ...
        const rectWallRight2 = BABYLON.MeshBuilder.CreateBox("rectWallRight2", { width: 1.6, height: 10, depth: 21.1 }, scene);
        ...
        const wallConnector = BABYLON.MeshBuilder.CreateBox("wallConnector", { width: wallThickness, height: wallHeightBox+70, depth: 80 }, scene);
        ...
        const wallEndLeftExtra2 = BABYLON.MeshBuilder.CreateBox("wallEndLeftExtra2", { width: 4.1, height: wallHeightBox + 1, depth: 55 }, scene);
        ...
        const wallEndRightExtra2 = BABYLON.MeshBuilder.CreateBox("wallEndRightExtra2", { width: 4.1, height: wallHeightBox + 1, depth: 55 }, scene);
       ...
       // ✅ Materiale marmo con striature nere
        ...
        const marmoTexture = new BABYLON.DynamicTexture("marmoTex", { width: 512, height: 512 }, scene);
        ...
        for (let s = 0; s < totalSteps; s++) {
            ...
            const box = BABYLON.MeshBuilder.CreateBox("stepNeg_" + s, {
                width: 1,
                height: h,
                depth: 15
            }, scene);
        ...
        for (let s = 0; s < totalSteps; s++) {
            const h = (s + 1) * (totalHeight / totalSteps);
            const box = BABYLON.MeshBuilder.CreateBox("stepPos_" + s, {
                width: 1,
                height: h,
                depth: 15
            }, scene);
        ...
        const grayMarmoTexture = new BABYLON.DynamicTexture("grayMarmoTex", { width: 512, height: 512 }, scene);
        ...
        const hexagon = BABYLON.MeshBuilder.CreateCylinder("hexagon", {
            height: 7,
        ...
        const hexagonMirror = BABYLON.MeshBuilder.CreateCylinder("hexagonMirror", {
            height: 7,
            diameter: 8,
            tessellation: 6
        }, scene);
...
BABYLON.SceneLoader.ImportMeshAsync("", "assets/models/", "floor.glb", scene).then((result) => {
        ...
        const raisedFloorCollider = BABYLON.MeshBuilder.CreateBox("raisedFloorCollider", {
            width: 112, height: 0.5, depth: 59
        }, scene);
        ...
        for (let s = 0; s < totalSteps; s++) {
            const h = (s + 1) * (totalHeight / totalSteps);
            const box = BABYLON.MeshBuilder.CreateBox("stepCenter_" + s, {
                width: 1,
                height: h,
                depth: 20
            }, scene);
        ...
        for (let s = 0; s < totalStepsSide; s++) {
            ...
            const box = BABYLON.MeshBuilder.CreateBox("stepSide_" + s, {
                width: spanX,
                height: h,
                depth: 1
            }, scene);
        ...
        BABYLON.SceneLoader.ImportMeshAsync("", "assets/models/", "floor.glb", scene).then((result) => {
            ...
            sideFloor.position = new BABYLON.Vector3(100, 4, startZ - totalStepsSide - 50.2);
            sideFloor.name = "sideFloor";
        });

        // ✅ Collider invisibile per il pavimento laterale
        const sideFloorCollider = BABYLON.MeshBuilder.CreateBox("sideFloorCollider", {
            width: 90, height: 0.5, depth: 40
        }, scene);
        sideFloorCollider.position = new BABYLON.Vector3(110.5, 4, startZ - totalStepsSide - 19);
        sideFloorCollider.isVisible = false;
        sideFloorCollider.checkCollisions = true;

        // ✅ Muri perpendicolari
        const perpWallTop = BABYLON.MeshBuilder.CreateBox("perpWallTop", {
            width: 104, height: wallHeightBox + 1, depth: wallThickness + 0.2
        }, scene);
        ...
        const perpWallTop1 = BABYLON.MeshBuilder.CreateBox("perpWallTop1", {
            width: 104, height: wallHeightBox + 1, depth: wallThickness + 0.2
        }, scene);
        ...
        const connectorWall = BABYLON.MeshBuilder.CreateBox("connectorWall", {
            width: 0.8,
            height: wallHeightBox + 100,
            depth: 53.5 +30
        }, scene);
        connectorWall.position = new BABYLON.Vector3(252, extraY, ((28.1 + (-25.4)) / 2));
        ...
        for (let i = 0; i < newColumnPositions.length - 1; i++) {
            ...
            [15, -16].forEach(zPos => {
                ...
                const wallBox2 = BABYLON.MeshBuilder.CreateBox("wall2_" + i + zPos, {
                    width: currentWallWidth,
                    height: wallHeight,
                    depth: currentDepth
                }, scene);
                wallBox2.position = new BABYLON.Vector3(finalMidX, archBaseY2 + (wallHeight / 2), finalZ);

                // ✅ Diametro arco ridotto
                const currentArchDiameter = isLeft ? 9 : 8;

                const archHole2 = BABYLON.MeshBuilder.CreateCylinder("archHole2_" + i + zPos, {
                ...
                const finalWall2 = resultCSG2.toMesh("finalWall2_" + i + zPos, wallMaterial, scene);
        ...
        for (let i = 0; i < columnPositions.length - 1; i++) {
            ...
            [25.6, -17.3].forEach(finalZ => {
                ...
                const fillBox = BABYLON.MeshBuilder.CreateBox("fillBox_" + i + finalZ, {
                    width: colWidth,
        ...
        const gapFill = BABYLON.MeshBuilder.CreateBox("gapFill", {
        ...
        const gapFill2 = BABYLON.MeshBuilder.CreateBox("gapFill2", {
            width: 19,
        ...
        const boxFill = BABYLON.MeshBuilder.CreateBox("boxFill", {
            width: 22,
            height: 10,
            depth: 20
        }, scene);
        ...
        const boxFill2 = BABYLON.MeshBuilder.CreateBox("boxFill2", {
            width: 22,
            height: 10,
            depth: 20
        }, scene);
...
// ✅ Taglia metà inferiore vault
const cutBoxVolta = BABYLON.MeshBuilder.CreateBox("cutBoxVolta", {
    width: 400, height: 40, depth: 400
}, scene);
...
const cutVoltaCSG = BABYLON.CSG.FromMesh(cutBoxVolta);
voltaCSG = voltaCSG.subtract(cutVoltaCSG);
...
// ✅ Taglia metà inferiore cupola
const domeCutBox = BABYLON.MeshBuilder.CreateBox("domeCutBox", {
    width: 100, height: 30, depth: 100
}, scene);
...
const domeCutCSG = BABYLON.CSG.FromMesh(domeCutBox);
domeCavCSG = domeCavCSG.subtract(domeCutCSG);

// ✅ Rimuovi dalla cupola la parte che entra nei vault
